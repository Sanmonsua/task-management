...
describe('User reducer', () => {
	it('signIn action', () => {
		expect(
			reducer(DEFAULT_STATE, actions.signIn(mockUser))
		).toMatchSnapshot()
	})
})
...
describe('Categories reducer', () => {
	...
	it('fetchCategories action', () => {
		...
		expect(
			reducer(DEFAULT_STATE, mockDispatch.mock.calls[0][0])
		).toMatchSnapshot()
	})

	it('AddCategory action', () => {
		expect(
			reducer(DEFAULT_STATE, actions.addCategory(mockCategory))
		).toMatchSnapshot()
	})

	it('AddTask action', () => {
		expect(
			reducer(DEFAULT_STATE, actions.addTask(mockTask))
		).toMatchSnapshot()
	})

	it('Edit category action', () => {
		expect(
			reducer(DEFAULT_STATE, actions.editCategory(mockCategory))
		).toMatchSnapshot()
	})
})
